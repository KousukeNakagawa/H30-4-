...
public class TutorialShotingFailure : MonoBehaviour {
    ...
    {
        if (!TutorialEnemyScripts.shootingPhaseMove.isShooting) return;

        TutorialEnemyScripts.bigEnemyAnimatorManager.AnimatorReset();
        targetPos.gameObject.SetActive(true);
        launchCount = launchTime;
        isFailure = true;
        TutorialEnemyScripts.shootingPhaseMove.isShooting = false;
    }
