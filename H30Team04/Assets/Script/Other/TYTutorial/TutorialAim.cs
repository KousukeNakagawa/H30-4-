...
public class TutorialAim : MonoBehaviour {
    ...
    {
        // 武器ごとの標準の表示 ON / OFF
        aimB.SetActive(TutorialWepon.WeaponBeacon);
        aimR.SetActive(!TutorialWepon.WeaponBeacon);
    }
    ...
    public static void AimMotion(Vector3 angle, Vector3 pos)
    {
        ...
        transform_.localScale = (!TutorialPlayer.IsMove && !TutorialPlayer.IsRotate) ?
        ...
        if (!TutorialPlayer.IsMove && !TutorialPlayer.IsRotate) return;

        // 位置
        transform_.position = pos + angle/10;
        // 基本の向き
